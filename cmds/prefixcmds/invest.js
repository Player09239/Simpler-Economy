...
**>** **Your Cookies** ${await format(u.cookies)} 🍪
            ...
            const err1 = new EmbedBuilder()
                .setTitle('Error')
                .setColor('Red')
                .setDescription(`Arguments missing, expected: **${b.prefix}invest <amount>**`)
                .setTimestamp()
            ...
            if (args[0].toLowerCase() === "all") {
                ...
                if (amount <= 0) {
                    const errAll = new EmbedBuilder()
                        ...
                        .setTimestamp();
                    return message.channel.send({ embeds: [errAll] });
                }
            } else {
                if (isNaN(args[0])) {
                    return message.channel.send({ embeds: [err2] })
                }
                amount = Math.floor(parseInt(args[0]));
                if (amount <= 0) {
                ...
                if (amount > u.cookies) {
                    const err4 = new EmbedBuilder()
                        ...
                        .setTimestamp()
                    return message.channel.send({ embeds: [err4] })
                }
            }
